...
* It must somehow keep track of each possible state, and which other states it can transition to.
* It must define when a transition is valid.
* It must provide a method to transition to a new state. This method will regularly be called by the State classes, and is then responsible for checking if the transition is valid, if it should happen, and if so, to transition to the new state.
