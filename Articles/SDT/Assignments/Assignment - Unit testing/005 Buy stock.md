...
I recommend a new file for these tests. Name it something meaningful, so it's clear what it is for.
...
For example, the `UnitOfWork.commit()` should be called once. This can be verified. The simple approach is to have the mock UoW contain a counter field variable, and increment it each time `commit()` is called.
